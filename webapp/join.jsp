...
<script>
    ...
    function joinSubmit() {
        // 유효성 검사
        if(document.getElementById("uid").value.length < 4) { alert("아이디를 4자 이상 입력하세요."); return; }
        if(document.getElementById("upw").value.length < 4) { alert("비밀번호를 4자 이상 입력하세요."); return; }
        if(document.getElementById("upw").value !== document.getElementById("upw_check").value) { alert("비밀번호가 일치하지 않습니다."); return; }
